...
// a 409 (`api/routers/auth.py`) -- matched by text rather than status
// code because `unwrap()` (queries.ts) only ever throws the parsed error
// body, not the response object.
...
function VerifySignupContent() {
  ...
  const isUsernameConflict =
    consume.isError && errorMessage(consume.error, "") === USERNAME_TAKEN_DETAIL;

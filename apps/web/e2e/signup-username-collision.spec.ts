...
/** A second signup deliberately colliding with an existing username gets
 * a 409 on consume and succeeds on retry with a different one, all
 * against the *same* emailed token (no second email needed) --
 * `auth.py::consume_signup`'s own contract. */
test("a colliding username on consume gets a 409 and succeeds after retrying with a new one", async ({
  ...
  // Second signup deliberately reuses the now-taken username -- still on
  // `/`, which re-renders as the onboarding form once `me` clears.
